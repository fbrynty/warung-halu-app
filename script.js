...
function setupChatHistoryListener(firestoreDb, userId, appId) {
    ...
    const chatCollectionRef = collection(firestoreDb, `artifacts/${appId}/users/${userId}/chat_history`);
    const q = query(chatCollectionRef, orderBy('timestamp', 'desc')); // Order by desc for latest message at index 0

    onSnapshot(q, (snapshot) => {
        if (!chatHistoryDiv) return; // Pastikan elemen ada
        ...
        if (snapshot.empty) {
            ...
            chatHistoryDiv.innerHTML += '<p>Belum ada obrolan. Ketik pesan untuk memulai.</p>'; // Tampilkan pesan placeholder
        ...
        const messages = snapshot.docs.map(doc => doc.data()).reverse(); // Reverse to display in chronological order
    ...
    // Fungsi baru untuk menampilkan pesan bot sementara (tidak disimpan ke Firestore)
    function displayTemporaryBotMessage(messageText) {
        ...
        chatHistoryDiv.scrollTop = chatHistoryDiv.scrollHeight; // Gulir ke bawah
    }
...
function adjustQuantity(itemName, delta) {
    ...
    if (!menuItem || menuItem.tersedia === false) {
        // Mengganti alert dengan pesan sementara di UI
...
function updateCartDisplay() {
    if (!cartItemsDiv || !cartTotalPriceSpan || !confirmOrderButton) { // Hapus cartShippingPriceSpan
    ...
    if (confirmOrderButton) {
        confirmOrderButton.addEventListener('click', () => {
            if (cart.length === 0) {
                ...
                if (cartSummary) { // Check if element exists
    ...
    if (konfirmasiFormModal) {
        konfirmasiFormModal.addEventListener('submit', async (event) => {
            ...
            const email = modalEmailInput ? modalEmailInput.value : ''; // Ambil email juga

            customerInfoForChat = { nama, whatsapp, alamat, catatan, email }; // Simpan email
            ...
            try {
                ...
                // Total sama dengan subtotal karena ongkir dihilangkan
                const orderDetails = {
                    customer: customerInfoForChat,
                    items: cart,
                    subtotal: subtotal, 
                    total: subtotal, // Total sama dengan subtotal
                ...
                if (response.ok) {
                    ...
                    if (konfirmasiModalLoading) {
                        ...
                        if (backToMenuButton) {
                            backToMenuButton.addEventListener('click', () => {
                                if (konfirmasiModal) konfirmasiModal.style.display = 'none';
                                // Tampilkan widget chatbot dan gulir ke sana
                                if (chatbotWidget) chatbotWidget.classList.remove('hidden');
                                if (kontakSection) { // Gulir ke bagian kontak jika ada
                                    kontakSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
                                }
                                // Isi chat input dengan pesan default
                    ...
                    // Display temporary error message for server-side issues
                ...
                // Display temporary error message for network/client-side issues
...
if (chatIcon && chatbotWidget && closeChatButton) {
    const openChat = () => {
        if (chatbotWidget) chatbotWidget.classList.remove('hidden');
        // Gulir ke bagian kontak jika widget dibuka dari icon
        if (kontakSection) { 
            kontakSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
    };
